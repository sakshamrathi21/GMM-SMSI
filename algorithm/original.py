...
while abs((np.asarray(meanPrev) - np.asarray(means)).sum()) > 0.01:
    ...
    smsi_mean_num = []
    smsi_mean_num_s_x = ndimage.generic_filter(smsi_mean_num_s_x.reshape(o_shape[0], o_shape[1]), np.nansum, footprint=np.ones((3, 3)), mode='constant', cval=np.NaN).reshape(-1)
    # print(np.shape(smsi_mean_num_s_x))
    for i in range(k):
